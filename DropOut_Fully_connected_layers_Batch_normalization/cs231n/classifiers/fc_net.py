...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        #first Layer forward
        out_hidden,cache = affine_relu_forward(x=X, w=w1, b=b1)
        #second layer
        scores, cache_score = affine_forward(x=out_hidden, w=w2, b=b2)
        ...
        ############################################################################
        lossOfData,dscores = softmax_loss(scores, y=y)
        reg_loss = 0.5 * self.reg * np.sum(w1**2)
        reg_loss += 0.5*self.reg * np.sum(w2**2)
        loss = reg_loss + lossOfData
        #last layer back
        dx1, dw2, db2 = affine_backward(dscores,cache_score)
        dw2 += dw2 * self.reg
        #1st layer back
        dx, dw1, db1 = affine_relu_backward(dout=dx1, cache=cache)
        dw1 += self.reg * dw1
...
class FullyConnectedNet(object):
    ...
    def loss(self, X, y=None):
        ...
        for i in xrange(1, self.num_layers):
            layer[i],cache_layers[i] = affine_relu_forward(layer[i-1],
        ...
        # of 0.5 to simplify the expression for the gradient.                      #
        ############################################################################
        loss, dscores = softmax_loss(scores,y=y)

        #add regularization loss into the loss
        for i in xrange(1,self.num_layers+1):
            loss += 0.5 * self.reg * np.sum(self.params['W%d'%i]**2)

        #backpropogation from last layer
        dx = {}
        ...
        for i in reversed(xrange(1,self.num_layers)):
            dx[i], grads['W%d'%i],grads['b%d'%i] = affine_relu_backward(dx[i+1],cache_layers[i])
